...
const UpdateAmc = () => {
  ...
  useEffect(() => {
    if (amcData) {
      setFormData({
        planName: amcData.serviceAMC || "",
        planPrice: amcData.price?.replace("₹", "").replace(".00", "") || "",
        duration: amcData.warrenty || "",
        discount: amcData.discount?.replace("%", "") || "",
        description: amcData.description || "Complete annual maintenance with priority support and regular checkups",
      ...
      if (amcData.images && amcData.images.length > 0) {
        setExistingImages(amcData.images);
      }
    }
  }, [amcData]);
