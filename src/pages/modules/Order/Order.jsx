...
const getStatusLabel = (status) => {
  if (status === "pending") return "New";
  if (status === "delivered") return "Delivered";
  return "In-Progress"; // assigned, confirmed, shipped, out_for_delivery
};
  ...
  const matchesSearch =
    ...
    o.orderedDate?.toLowerCase().includes(search.toLowerCase()) ||
...
const statusBadgeClass = (status) =>
  status === "pending"
    ...
    : status === "delivered"
    ? "text-green-600"
    : "text-blue-500"; // assigned, confirmed, shipped
            ...
            <table className="w-full border-collapse min-w-[500px] text-center">
              ...
              <tbody>
                {ordersPaginated.map((o, idx) => (
                  <tr key={o._id} className="border-t border-gray-300  transition-all duration-200">
                    ...
                    <td className="p-3 align-middle">{o.customer?.name}</td>

                    {/* <td className="p-3 align-middle">{o.product}</td> */}
                    <td className="p-3 align-middle">{o.items?.[0]?.name}</td>

                    {/* <td className="p-3 align-middle">{o.orderDate}</td> */}
                    <td className="p-3 align-middle">{new Date(o.orderedDate).toLocaleDateString()}</td>
                    ...
                    <td className="p-3 align-middle">
                      <div className="flex justify-center items-center">
                        <button
                          ...
                          //  onClick={() => navigate(`/order-management/view/${o.orderId}`, { state: o })}
                          onClick={() => navigate(`/order-management/view/${o._id}`)}
          ...
          {ordersPaginated.map((o, idx) => (
            <div key={o._id} className="bg-white border rounded-lg p-4 shadow-sm">
              <div className="flex items-start justify-between gap-3">
                ...
                <div>
                  <span className={`px-2 py-1 rounded-full text-sm font-medium ${statusBadgeClass(o.orderStatus)}`} >
                    {/* {o.status} */}
                    {getStatusLabel(o.orderStatus)}
              ...
              <div className="mt-3 text-sm text-gray-700">
                <div className="mb-1"><span className="font-medium">Customer:</span> {o.customer?.name}</div>
                <div className="mb-1"><span className="font-medium">Product:</span> {o.items?.[0]?.name}</div>
                <div className="mb-1"><span className="font-medium">Order Date:</span>{" "} {new Date(o.orderedDate).toLocaleDateString()}</div>
              </div>
